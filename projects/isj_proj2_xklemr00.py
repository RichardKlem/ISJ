...
def solfege(title_hymn):
    ...
    possible_title, sep, hymn = re.findall(r'(.*)(?=:)', title_hymn), re.findall(r':', title_hymn), re.findall(
        r'(?<=: )(.+)|(^((?!:).)*$)', title_hymn)  # vase reseni

    # the hymn as a list of strings separated by ' '
    hymn_list = re.findall(r'[A-z]+', str(hymn))  # vase reseni

    # skipping always two strings, and ending 3 strings from the end
    skip2 = hymn_list[1:len(hymn_list) - 4:3]  # vase reseni
